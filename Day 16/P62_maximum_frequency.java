...
class P6_maximum_frequency {
    public static void main() {
        ...
        for (int i = 0; i < n; i++) // loop to calculate the frequency of each element in the array
        {
            ...
            for (int j = 0; j < n; j++) {
                if (arr[i] == arr[j]) // if the current element is equal to the element at index j, increment the
                                      // frequency
                {
                    freq++;
                }
            }
            if (freq > max_freq) // if the frequency of the current element is greater than the maximum
                                 // frequency, update the maximum frequency and the number with maximum frequency
            {
                max_freq = freq;
                num = arr[i];
            }
